	...
	useEffect(() => {
		if (isNavOpen) {
			if (expandableDivRef.current)
				expandableDivRef.current.style.left = "25%";
			if (mobileNavMenuButtonSpan.current)
				mobileNavMenuButtonSpan.current.style.transform =
					"translateY(-50%)";
		} else {
			if (expandableDivRef.current)
				expandableDivRef.current.style.left = "100%";
			if (mobileNavMenuButtonSpan.current)
				mobileNavMenuButtonSpan.current.style.transform =
					"translateY(0%)";
		}
				...
				<nav className="fixed w-full flex items-center justify-between py-4 px-8 z-50">
					...
					</button>
					{
						<div
							...
							className="fixed right-0 left-full bottom-0 top-0 z-40 transition-all bg-blur-transp border-blur-transp border-2 backdrop-blur-sm"
						>
				...
				<div className="flex flex-col justify-between h-full">
					...
					<div className="h-full grid grid-cols-3 md:grid-cols-2">
						<div className="place-self-center flex gap-16 items-center col-start-1">
							...
							<h1 className="text-3xl font-semibold hidden md:inline">
								WATCH.IT
							</h1>
						</div>
